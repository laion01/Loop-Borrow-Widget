...
import SWAP_ICON from '../images/refresh.svg'
...
export const FormContainer = ({
  ...
  const invalid =
    ...
    && !gt(uusd, uusdAmount)
      ? [`Not enough ${msgInfo.symbol ?? "balance"}`]
      : undefined
  ...
  const render = (children: ReactNode | ((button: ReactNode) => ReactNode)) => {
    ...
    const form = (
      <>
        ...
        {gt(uusd, div(plus(!deduct ? tax : 0, fee.amount), SMALLEST)) && contents && (
          <Confirm
            list={[
              ...contents,
              {
        ...
        {gt(uusd, div(plus(!deduct ? tax : 0, fee.amount), SMALLEST)) && (invalid ?? messages)?.map((message, index) => (
          <FormFeedback key={index}>{message}</FormFeedback>
        ))}
        {
            !gt(uusd, div(plus(!deduct ? tax : 0, fee.amount), SMALLEST)) && <UstNotEnough uusdAmount={txFee} />
        }

        { 
        showSubmitBtn && <Button {...next} icon={SWAP_ICON} type="button" size="lg" submit  />}
